...
if(!isset($_GET["projetASupprimer"])) { // on verifie que nous avons bien l'identifiant de notre menu à supprimer.
    ajouterErreur("Merci de choisir le projet à supprimer.");
} else {
    // on a l'identifiant, nous supprimons le menu
    $bdd -> query("DELETE FROM menu WHERE id_menu = " . $_GET["projetASupprimer"]);
    ajouterSuccess("Le projet a été supprimer.");
